...
class SWSeMaskBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        K = self.n_cls
        # print(x.shape)
        shift_feat = self.shift_feat(x, n_div=4)
        # print(x_shift.shape)

        x_shift = rearrange(shift_feat, 'a b c d -> (a c d) b')
        x = x_shift + self.norm1(x_shift)
...
class D2SeMaskSwinTransformer(SeMaskSwinTransformer):
