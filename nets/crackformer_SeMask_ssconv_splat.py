...
# from ..SeMask_backbone.semask_swin_shift import *
from .SeMask_backbone.semask_swin import *
from .sconv import ScConv
...
class FRB(nn.Module):
    ...
    def forward(self, down_inp, up_inp, size, attention):
        ...
        outputs1 = self.ScConv(outputs)
        outputs = self.conv(outputs + outputs1)
...
class Up1(nn.Module):
    ...
    def forward(self, inputs, indices, output_shape):
        ...
        scale1_4 = scale1_3 + self.splat(scale1_4)
...
class Up2(nn.Module):
    ...
    def forward(self, inputs, indices, output_shape):
        ...
        scale2_4 = self.Conv3X3(scale2_3) + self.splat(scale2_4)
...
class Up3(nn.Module):
    ...
    def forward(self, inputs, indices, output_shape):
        ...
        scale3_6 = self.Conv3X3(scale3_5) + self.splat(scale3_6)
...
class Up4(nn.Module):
    ...
    def forward(self, inputs, indices, output_shape):
        ...
        scale4_6 = self.Conv3X3(scale4_5) + self.splat(scale4_6)
...
class Up5(nn.Module):
    ...
    def forward(self, inputs, indices, output_shape):
        ...
        scale5_6 = scale5_5 + self.nn3(scale5_6)
...
class crackformer(nn.Module):
    ...
    def __init__(self):
        ...
        self.SWSeMaskBlock_64 = SWSeMaskBlock(dim=64, n_cls=1)#dim, n_cls
        self.SWSeMaskBlock_128 = SWSeMaskBlock(dim=128, n_cls=1)#dim, n_cls
        self.SWSeMaskBlock_256 = SWSeMaskBlock(dim=256, n_cls=1)#dim, n_cls
        self.SWSeMaskBlock_512 = SWSeMaskBlock(dim=512, n_cls=1)#dim, n_cls
        self.SWSeMaskBlock_512_0 = SWSeMaskBlock(dim=512, n_cls=1)#dim, n_cls
    ...
    def forward(self, inputs):
        ...
        out_256 = rearrange(out_256, 'a b c d -> a d b c')
        out_512 = rearrange(out_512, 'a b c d -> a d b c')
        out_512_0 = rearrange(out_512_0, 'a b c d -> a d b c')

        out_128 = F.interpolate(out_128, size=(256, 256), mode='bilinear', align_corners=True)
        out_256 = F.interpolate(out_256, size=(256, 256), mode='bilinear', align_corners=True)
        out_512 = F.interpolate(out_512, size=(256, 256), mode='bilinear', align_corners=True)
        out_512_0 = F.interpolate(out_512_0, size=(256, 256), mode='bilinear', align_corners=True)

        # Sum all upsampled tensors along dim=0
        se_mask_out = torch.cat([out_64, out_128, out_256, out_512, out_512_0], dim=1)
